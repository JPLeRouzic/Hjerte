        ...
        if (event.getSource().equals(extract_features_button)) {
            ...
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    ...
                    String similScore = EntryPoint.hmmTest.getSimilarity();
                    ...
                    ArrayList due = EntryPoint.hmmTest.getObsrvSeq();
                    for (int idx = 0; idx < EntryPoint.hmmTest.worksWell.size(); idx++) {
                        String str = (String) EntryPoint.hmmTest.worksWell.get(idx);
                        Character trez = str.charAt(1);
                        String yon = trez.toString();
                        if (Integer.parseInt(yon) < 5) {
                            simi.textArea.append("\n");
                            simi.textArea.append(str.substring(0, 2));
                            simi.textArea.append(",    ");
                            Observation undex = (Observation) due.get(idx) ;
